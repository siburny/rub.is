...
class Numword {
	...
	public static $digits_ord = array('zero', 'first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'ninth');
	...
	private static function __convert($number) {
		...
		if ($length > 3) {
			...
			$return = !self::$ordinal ? self::$digits[$number] : self::$digits_ord[$number] ;
		}
	...
	private static function __convertDoubles($number) {
		...
		if ($fn == 1 && $ln != 0) {
			...
			$return = !self::$ordinal ? self::$digits[$ln] : self::$digits_ord[$ln];
		} else {
			$return = self::$tens[$fn . '0'] . self::$sep . (!self::$ordinal ? self::$digits[$ln] : self::$digits_ord[$ln]);
		}
